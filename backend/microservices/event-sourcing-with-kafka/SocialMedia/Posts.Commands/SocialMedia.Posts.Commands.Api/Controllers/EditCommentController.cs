...
using SocialMedia.Posts.Commands.Api.Features.EditComment;
...
public class EditCommentController : ControllerBase {
    ...
    private readonly ICommandDispatcher<EditCommentCommand> _commandDispatcher;

    public EditCommentController(ILogger<EditCommentController> logger, ICommandDispatcher<EditCommentCommand> commandDispatcher) {
        this._logger = logger;
        this._commandDispatcher = commandDispatcher;
    }

    [HttpPut("{id}")]
    public async Task<ActionResult> EditCommentAsync(Guid id, EditCommentCommand command, CancellationToken cancellationToken) {
        try {
            command.PostId = id;
            await this._commandDispatcher.SendAsync(command, cancellationToken);

            return Ok(new BaseResponse("Edit comment request completed successfully!"));
        }
